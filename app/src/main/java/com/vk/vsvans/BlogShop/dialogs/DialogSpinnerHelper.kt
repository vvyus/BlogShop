...
import com.vk.vsvans.BlogShop.adapters.DialogSpinnerSellerAdapter
...
class DialogSpinnerHelper {
    fun showSpinnerSellerDialog(context: Context, list: ArrayList<Seller>, tvSelection: TextView){
        val builder=AlertDialog.Builder(context)
        val dialog=builder.create()
        //надуваем в диалоге нет инфлайтора поэтому берем его из контекста .from
        val rootView=LayoutInflater.from(context).inflate(R.layout.spinner_layout,null)
        val adapter= DialogSpinnerSellerAdapter(tvSelection,dialog)
        ...
        setSearchViewSeller(adapter,list,sv)

        dialog.show()
    }
    private fun setSearchViewSeller(adapter: DialogSpinnerSellerAdapter, list: ArrayList<Seller>, sv: SearchView) {
        if (sv != null) {
            sv.setOnQueryTextListener(object:SearchView.OnQueryTextListener{
                ...
                override fun onQueryTextChange(newText: String?): Boolean {
                    val tempList= SpinnerHelper.filterListDataSeller(list,newText)
